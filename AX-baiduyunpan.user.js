...
(function(require, define, Promise) {
	...
	unsafeWindow.axeldown = function(urls) {
		urls = urls.split('|');
		//var thsize = thread;
		for (var i in urls) {
			var url = urls[i];
	...
	function showError(msg) {
		...
		var $dialog = $('<div id="errorDialog">' + '<h3>EX-baiduyunpan:程序异常</h3>' + '<div class="dialog-body"><p>请尝试更新脚本或复制以下信息提交issue</p>' + '<p>Exception: ' + msg + '</p>' + '<p>Script Ver: ' + GM_info.script.version + '</p>' + '<p>TemperMonkey Ver: ' + GM_info.version + '</p>' + '<p>UA: ' + navigator.userAgent + '</p>' + '</div><hr><a class="close" href="javascript:;">关闭</a></div>');
	...
	define('ex-yunpan:pageInfo', function(require) {
		...
		var PAGE_CONFIG = {
			...
			share: {
				...
				style: function() {
					var styleList = ['.KPDwCE .QxJxtg{z-index: 2;}', '.module-share-header .slide-show-right{width: auto;}', '.ex-yunpan-dropdown-button.g-dropdown-button.button-open .menu{z-index:41;}', '.module-share-header .slide-show-header h2{width:230px;}', '.KPDwCE .xGLMIab .g-dropdown-button.ex-yunpan-dropdown-button{margin: 0 5px;}'];
			...
			enterprise: {
				...
				style: function() {
					var styleList = ['.ex-yunpan-dropdown-button .icon-download{background-image: url(/box-static/business-function/infos/icons_z.png?t=1476004014313);}', '.ex-yunpan-dropdown-button .g-button:hover .icon-download{background-position: 0px -34px;}'];
					GM_addStyle(styleList.join(''));
				}
			}
		};
				...
				'click': function() {
					...
					Promise.all(promises).then(function(result) {
						...
						dialog.buttonIns[0].dom.attr({
							'href': 'javascript:window.axeldown("' + urls + '");',
